...
const ORDER_SERVICE_URL = process.env.ORDER_SERVICE_URL || 'http://localhost:5001/api/orders';

class OrderIntegrationService {
  // Get all orders that are ready for pickup (need a driver)
  async getReadyForPickupOrders() {
    try {
      const response = await axios.get(`${ORDER_SERVICE_URL}/ready-for-pickup`);
      return { success: true, orders: response.data };
    } catch (error) {
      console.error('[OrderIntegrationService] Error fetching ready-for-pickup orders:', error.message);
  ...
  async getOrderById(orderId) {
    try {
      ...
      console.error(`[OrderIntegrationService] Error fetching order ${orderId}:`, error.message);
      return { success: false, order: null };
    }
  }

  // Update driver assignment info on an order
  async updateOrderDriverAssignment(orderId, assignmentData) {
    try {
      const response = await axios.put(`${ORDER_SERVICE_URL}/${orderId}/driver-assignment`, assignmentData);
      return { success: true, order: response.data };
    } catch (error) {
      console.error(`[OrderIntegrationService] Error updating driver assignment for order ${orderId}:`, error.message);
      return { success: false, order: null };
    }
  }
  ...
  async updateOrderStatus(orderId, status) {
    try {
      const response = await axios.put(`${ORDER_SERVICE_URL}/${orderId}/status`, { status });
      return { success: true, order: response.data };
    } catch (error) {
      console.error(`[OrderIntegrationService] Error updating status for order ${orderId}:`, error.message);
      return { success: false, order: null };
    }
  }
}
